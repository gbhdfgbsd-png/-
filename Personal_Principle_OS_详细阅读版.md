...
> 本版本为**详细阅读版**——在完整设计内容基础上，为每个重大设计决策附加设计推理注释（> 设计推理），帮助读者理解"为什么这样设计"。
...
> **设计推理：系统定义**
>
> **解决的问题：** 达里奥《原则》的APP只是"找到它→读它"的逻辑，本质上是检索式查阅手册，不是真正的决策辅助系统。用户需要的不是"一本可搜索的原则书"，而是一个能从行为中学习、能在决策时实时辅助、能追踪认知演化的活系统。
>
> **为什么这样定义（否决方案）：**
> - ~~知识库方案~~：把信息源切块存起来问什么搜什么——这不产生新认知，只是信息检索。
> - ~~格言收集册~~：收集金句不等于内化原则，缺乏验证机制等于自欺欺人。
> - ~~纯推理引擎~~：先有原则再推理的逻辑是倒因为果——原则应该从行为中涌现。
>
> **已知风险：** 系统复杂度高，冷启动阶段用户可能因缺乏即时反馈而放弃。通过30天分阶段启动计划缓解。
...
> **设计推理：涌现而非灌入**
>
> **解决的问题：** 用户是即将毕业的学生，个人决策沉淀几乎为零。原始方案试图"从外部高质量信息源填充原则库"来解决冷启动，但这本质上是用别人的原则假装是自己的——达里奥的原则之所以是原则，是因为每条都经过了真实的、有代价的决策验证。
>
> **为什么这样设计：** 科学方法论结构——观察现象→提出假说→设计实验验证→假说被证实或推翻。对应到系统：从信息源读到观点→格式化为"待验证假说"→真实决策中验证→结果好则升级。**别人的原则未经你验证不是你的原则。**
>
> **否决方案：** 从信息源直接提炼原则作为初始库——会变成确认偏误放大器。
>
> **用户驱动修改：** 用户确认接受"先记录30天再搭建原则库"的冷启动策略。
...
> **设计推理：张力驱动成长**
>
> **解决的问题：** 如果系统只追求"一致性"（让行为符合声称的原则），会变成一个自我批判工具。但现实中"你实际做的"和"你想做的"之间的差距才是最有价值的信息——它暴露的可能是执行力问题，也可能是原则本身需要更新。
>
> **核心洞察：** 原则的三种状态——描述性（你实际在做什么）、规范性（你希望怎么做）、张力（两者之间的裂缝）。系统核心价值在于持续暴露张力，帮你逐步缩小差距或更新原则。
>
> **否决方案：** 单一类型原则（不区分描述性/规范性）——无法产生张力，也就无法驱动成长。
...
> **设计推理：对手盘机制**
>
> **解决的问题：** 一个完全基于"我过去认同什么"构建的系统，天然会成为确认偏误放大器——输入的信息源都是认可的，提炼出的原则都是同意的，推理结论必然倾向已有认知框架。如果系统给的结论总是觉得"对，就这样"，那它只是在帮你更快地确认本来就会做的决策。
>
> **为什么这样设计：** 最有价值的原则往往来自你犯过的最痛的错误，而不是来自你认同的最聪明的人。系统必须主动提供反面论证，这才是真正的"决策质量提升"。
>
> **否决方案：** 系统给出确定结论——有确认偏误风险，且剥夺了用户决策自主权。
>
> **用户驱动修改：** 用户选择了"冲突性"而非"确定性"的决策辅助模式。
...
> **设计推理：零摩擦设计**
>
> **解决的问题：** Layer 0（决策日志）需要满足两个互相矛盾的要求——输入时极简（记录时不能感到负担），存储时丰富（后续分析需要足够多维度）。如果输入有格式要求，用户会因为负担而停止记录。
>
> **解决方案：** 你负责输入最少信息，系统负责补全和推断剩余字段。核心哲学：你负责"说"，系统负责"想"。系统不替你做判断，它把思考过程透明呈现，让你做最终确认。
...
> **设计推理：原则考古学**
>
> **解决的问题：** 原则不是静态的——22岁认同的原则，25岁可能觉得是胡扯。但那条原则被废弃的过程本身就是极有价值的信息。系统应该保留完整的"原则演化史"：什么时候被采纳、什么时候被修正/推翻、推翻它的是什么事件、推翻后被什么新原则替代。
>
> **为什么不能只保留"当前有效的"：** 五年后回头看，"认知演化路径"比任何单一时间点的原则库都有价值。退役原则也有用——决策辅助层在特殊场景可能提醒"你曾有相关原则但退役了"。
...
> **设计推理：双轨并行架构**
>
> **解决的问题：** 用户有两种完全不同的输入来源——自己的决策行为（source: SELF）和外部信息源的观点（source: EXTERNAL）。这两类数据性质完全不同，前者是事实，后者是待验证假说。
>
> **为什么分Layer 0和Layer 1：** 两条轨道的数据必须打上不同标签，在系统里永远不混为一谈。交汇点在于：当你记录决策时，系统自动检索假说库推送相关假说；决策结果反向验证/推翻假说。
>
> **为什么Layer 4和Layer 5平行而非串行：** 张力检测是持续后台运行（被动），决策辅助是用户主动触发（主动）。两者都依赖Layer 3但服务于不同场景。
>
> **否决方案：** 单轨结构（所有输入不区分来源）——会让外部观点和真实行为数据混淆，无法区分"你认同的"和"你实际做到的"。
...
> **设计推理：决策日志定位**
>
> **解决的问题：** 所有上层的模式识别、原则涌现、张力检测都依赖这一层的数据质量。但如果要求用户填写完整结构化数据，记录负担过重会导致系统废弃。
>
> **核心洞察（洞察1）：** 系统应该是"决策日志+原则涌现引擎"，而非"原则库+推理引擎"。把因果关系反过来——不是先有原则再用原则做决策，而是先有真实决策记录，从决策模式中涌现出原则。
>
> **为什么"你负责最少信息，系统负责补全"：** 满足两个互相矛盾的要求——输入时极简（不感到负担）+ 存储时丰富（后续分析需要多维度）。
...
> **设计推理：边界条件哲学**
>
> **解决的问题：** 现实中用户输入不会完美——信息不完整、频率过高、忘记回填、评价反转都是常态。系统必须优雅地处理所有非理想情况。
>
> **为什么"不阻断"是第一原则：** 如果输入不完整就阻断记录流程，用户会为了避免麻烦而停止记录。数据地基比数据质量更重要——有partial的数据总比没数据好。
>
> **为什么"评价反转"要保留所有版本：** 反转本身是极有价值的认知信号。"当时觉得对的，半年后觉得错了"——两个评价都保留，反转记录本身可能揭示认知盲区。
...
> **设计推理：试金石库定位**
>
> **解决的问题：** 用户有6个高质量外部信息源，观点密度高、有明确立场和推导逻辑。但直接把这些观点当作原则使用是危险的——未经真实决策验证的观点只是"你同意的东西"，不是你的原则。
>
> **命名为"试金石"的含义：** 你的真实决策才是试金石，大佬的观点是等待被检验的"金子"。看起来像金子，但到底是不是，只有现实生活能告诉你。
>
> **为什么不是笔记本/收藏夹：** 每条内容有明确生命周期——active→验证→升级为原则 或 被推翻退役。没有"永远躺着"的内容。
>
> **否决方案：** 将信息源观点直接作为原则来源——会变成确认偏误引擎。
>
> **用户确认的定位：** 信息源不是原则来源，信息源是原则的"预设假说"来源。
...
> **设计推理：TFRF质量框架**
>
> **解决的问题：** 什么叫"好假说"？如果没有质量标准，试金石库会充斥着"听起来有道理但永远用不上"的金句。系统需要一个客观标准来区分可操作假说和无效内容。
>
> **为什么这四个维度：**
> - **T-Trigger**解决"想不起来用"的问题——没有明确触发场景的假说等于不存在
> - **F-Falsifiability**解决"不可证伪"的问题——不可证伪的观点走value_orientation通道
> - **R-Resolution**解决"验证周期太长"的问题——如果需要10年才能验证，对当前决策没有帮助
> - **F-Fidelity**解决"格式化失真"的问题——确保系统处理没有歪曲原始逻辑
>
> **为什么不阻断入库：** C级以下呈现改进建议但不阻断——保持零摩擦原则。
...
> **设计推理：信息源冲突处理**
>
> **解决的问题：** 两条来自不同信息源的观点可能互相矛盾（如"长期主义"vs"速度就是一切"）。系统不应该让用户"判断哪个对"。
>
> **为什么不做二选一：** 更常见的情况是它们适用于不同的决策阶段或不同的人生阶段。如示例："选择做什么"层面→长期主义适用；"确定做什么后怎么执行"层面→速度优先适用。
>
> **设计方案：** 每条假说有`boundary_conditions`字段——每条原则/假说不是普遍适用的，有前提条件和适用范围。当检测到冲突时，系统问"这两条在什么条件下分别成立？它们的边界在哪？"
...
> **设计推理：模式引擎定位**
>
> **解决的问题：** 用户不知道自己的"真实操作系统"是什么——你以为自己重视深度工作，但数据可能显示62%的时段你选了碎片化。人无法通过内省准确认识自己的行为模式，需要系统从数据中客观识别。
>
> **为什么"纯粹吃L0数据"：** 模式引擎不接受任何主观输入（你认为自己的模式是什么），它只看客观行为数据。这保证了描述性原则的客观性——它们是你"实际在做的"，不是你"以为自己在做的"。
>
> **为什么需要用户确认才生效：** 防止误检/保持用户操控感。系统可能识别出伪模式（数据巧合），必须由用户验证。
>
> **否决方案：** 系统自动生成原则不需确认——误检风险高，且用户失去操控感。
...
> **设计推理："看到但不想改"的处理**
>
> **解决的问题：** 如果用户看到反模式（如"说要早睡但实际晚睡"）但明确表示"我知道但不想改"，系统不应该持续骚扰。
>
> **为什么不触发张力检测：** 张力的前提是"你想改但没做到"。如果明确接受现状，那不是张力——是你对自己的了解。但如果未来设定了冲突的规范性原则或结果持续为负，系统会重新激活。
>
> **已知风险：** 用户可能在情绪低落时对所有反模式都选"不想改"——月报会呈现dismiss数量作为元数据，不做评判。
...
> **设计推理：原则层定位**
>
> **解决的问题：** 原始设计缺少"原则冲突"的处理机制。现实中高质量决策的难点从来不是"我没有原则可以参考"，而是两条你都认同的原则在同一个场景里互相矛盾。如果没有优先级体系和冲突裁决机制，系统输出的就不是"决策"，而是"这几条原则分别说了什么"。
>
> **为什么是"活体结构"而非"平面列表"：** 原则之间有层级（元原则→原则→子原则）、有关系（支持/冲突/替代）、有生命周期（创建→修订→退役）。平面列表无法表达这些关系，也无法支撑冲突裁决。
...
> **设计推理：冲突裁决机制**
>
> **解决的问题：** 凌晨两点，你已经工作了10小时，身体发出警告。原则A说"每天有效工作时间≥12小时"，原则B说"身体是所有产出的底层基础设施，健康优先级最高"——调用哪条？
>
> **为什么需要优先级体系：** 没有优先级的原则库遇到冲突就瘫痪。Tier系统确保生存安全绝对优先，核心方向次之，日常标准再次。
>
> **为什么不自动裁决：** 同级冲突中系统无法代替用户做价值判断。系统的价值是"让冲突变得清晰可见"——明确冲突点、检查边界、检查前提——剩下的由你做最终裁决。
>
> **为什么记录裁决结果：** 同类冲突反复出现且裁决一致→说明存在未显性化的"仲裁原则"→建议增加。
...
> **设计推理：情境相变模型**
>
> **解决的问题：** 原始问题是"原则置信度的情境衰减——人生阶段变化后旧原则是否需要re-validation"。错误模型是线性时间衰减（原则越老越不可靠）。
>
> **为什么不用线性时间衰减：** 时间本身不影响有效性——一条原则10年前验证的，如果情境没变（你还是同一个人、同样的生活条件），它一样有效。真正影响有效性的是**情境变化**（毕业/换工作/搬城市/收入骤变）。
>
> **情境相变检测：** 显式（用户声明"我毕业了"）+ 隐式（系统从数据推断：time_pressure分布突变/新维度出现/financial维度频率×3）。
>
> **Re-validation机制不是单独任务：** 嵌入正常使用——原则下次自然触发时附带情境提醒。不增加用户负担。
>
> **否决方案：** 线性时间衰减——时间本身不影响有效性，情境变化才影响。
...
> **设计推理：张力的重新定义**
>
> **解决的问题：** 如果把张力定义为"两条原则文本上的逻辑矛盾"，会产生大量伪张力（文本矛盾但从未在同一场景中被同时调用），同时遗漏真正的张力（看似无关的原则因真实行为暴露了隐藏取舍）。
>
> **核心定位校准：** 张力是行为与意图之间的裂缝在特定时间窗口内的累积。输入不是"两条原则的文本"，而是"一个行为序列 + 一条规范性原则 + 时间窗口"。
>
> **为什么"累积偏离"而非"单次偏离"：** 单次不遵循不是张力，是例外。系统抓趋势不抓个案——偶尔一次焦虑决策不算，30天内4/5次焦虑决策才是张力。
>
> **否决方案：** 原则文本对比（检测逻辑矛盾）——文本矛盾≠实际张力。
...
> **设计推理：四种张力来源**
>
> **解决的问题：** 如果只检测"行为-意图张力"（你说A做B），会遗漏其他三种同样重要的张力信号。
>
> **为什么"演化-锚定张力"最有价值：** 前三种本质上都是"你没做到你说的"。但第四种揭示的是"你已经变了但操作系统没跟上"——这不是你需要被纠正，而是原则需要被更新。这是整个系统最有价值的洞察类型之一。
>
> **已知风险：** 演化-锚定张力最难检测（需要模式引擎的"相变检测"配合原则最后更新时间的对比）。初期数据不足时可能产生误报。
...
> **设计推理：张力报告的语气设计**
>
> **解决的问题：** 如果张力报告读起来像"绩效考核不合格通知"，用户会抗拒使用系统。张力检测不是纪律工具。
>
> **为什么"镜子"而非"法官"：** 系统呈现数据和模式分析，提出假设（"你可能隐含地区分了……"），但不做结论。用户可能选择"缩小差距"，也可能选择"更新原则"——两种都是正确的响应。
>
> **为什么提供5种响应选项：** 覆盖所有合理响应——想改（accept_tension）、不想改是原则该更新（accept_behavior）、需要加边界（add_boundary）、系统搞错了（dismiss）、以后再说（defer）。没有"你错了"的暗示。
...
> **设计推理：张力健康度**
>
> **解决的问题：** 如果系统追求"零张力"，会导致用户要么不断降低标准（让规范性原则退化为描述性原则），要么产生挫败感（觉得自己永远做不到）。
>
> **为什么健康水平是2-4条：** 张力=0说明你没有在挑战自己（标准太低或检测太松）。张力=5+说明标准太多不现实。2-4条是"有成长空间但不至于崩溃"的平衡点。
>
> **核心哲学：** 张力是成长引擎，不是考核指标。系统的目标不是消除张力，而是帮你"选择性地缩小最重要的张力"。
...
> **设计推理：Layer 5为什么最难设计**
>
> **解决的问题：** Layer 0-4是记录和分析（处理已发生的事）。Layer 5是唯一要在决策发生之前实时工作的模块。约束组合最极端：时间压力（用户可能5分钟内就要做决定）、多原则整合（15-20条原则被同时调用怎么整合）、不能给结论（对手盘哲学）、要给对手盘（不能只列举正面）。
>
> **核心难题：** 多条原则被同时调用时，怎么从"列举每条原则分别怎么说"升级到"整合出一个有结构的正反论证"？60分方案是列举式（列出每条说什么），90分方案是推理链合成（把多条原则串成推理链，每步标注依据+confidence）。
>
> **否决方案：** 列举式呈现（每条原则分别怎么说）——用户看完还得自己整合，等于系统做了一半工作。
...
> **设计推理：时间尺度升维**
>
> **解决的问题：** 用户可能陷入"反复在同一类微决策上消耗能量"的模式，而没有意识到真正需要做的是一个策略级决策。
>
> **为什么"不强制升维"：** 有时候用户确实只需要今晚的答案（临时紧急情况）。系统只指出可能性，不代替用户判断是否需要升级讨论层次。
>
> **已知风险：** 升维建议可能让用户觉得"我只是想快速得到答案，系统却让我思考更大的问题"——通过"继续当前决策"选项保留快速路径。
...
> **设计推理：对抗性筛选**
>
> **解决的问题：** 如果原则召回的结果全指向同一选项，系统就退化为"确认偏误引擎"。对手盘哲学要求即使所有原则一致，也必须有反面论证。
>
> **为什么"强制拉入反方向"：** 如果不强制，confirmation bias会让系统只呈现用户已经倾向的方向。分级策略：先从原则中找反方向→没有就从假说中找→连假说都没有→至少标记"体系在该方向有盲区"。
>
> **已知风险：** 强制拉入的反方向原则可能relevance很低，导致对手盘论证看起来牵强。通过confidence标注让用户知道反方论据的强度。
...
> **设计推理：论证链合成vs列举式**
>
> **解决的问题：** 60分方案（列举式）列出每条原则说什么，最后说"综合来看倾向A"——用户看完还得自己整合。90分方案（论证链合成）把多条原则串成推理链，每一步标注依赖哪条原则+confidence。
>
> **为什么需要weakest_link和strongest_point：** 决策质量的关键不在于论证长度，而在于识别"最脆弱的一环"——那是反方最有效的攻击点，也是你最应该深入思考的地方。
>
> **否决方案：** 列举每条原则观点——列举式用户还得自己整合，系统只做了搬运工作。
...
> **设计推理：Quick_test的本质**
>
> **解决的问题：** 在时间极紧（5分钟内要回复）的场景下，用户没有带宽进行完整的论证链分析。需要一个压力下可用的极简决策辅助。
>
> **为什么"调用情绪直觉"：** 压力下理性带宽不够——你不可能在5分钟内做NPV计算。但你可以在5秒内回答"3个月后的你会骂现在的你吗？"。本质是把复杂推理压缩成情绪探针，把你从自动驾驶中拽出来。
>
> **已知风险：** 情绪探针可能被当前情绪状态扭曲。但这好过在压力下完全凭直觉决策——至少多了一个"停顿"的机会。
...
> **设计推理：Layer 6的三重定位**
>
> **解决的问题：** 如果只做"历史记录"（记录什么变了），价值有限——用户不会经常翻看变更日志。Layer 6需要提供三种主动价值：叙事（帮你理解变化的因果）、预测（在你意识到之前发现变化征兆）、锚定（在所有变化中告诉你"什么没变"）。
>
> **为什么需要"身份内核检测"：** 所有系统都假设"变化=好"。但如果什么都在变，用户会失去"我是谁"的锚定。身份内核不是你"声称不变的"，是数据证明确实没变的——这是整个系统最深层的安全感来源。
>
> **已知风险：** 身份内核检测可能让用户"过于锚定"在已有模式上，阻碍必要的转变。通过"成长vs漂移"区分机制缓解。
...
> **设计推理：趋势预测的价值**
>
> **解决的问题：** 正常情况下，从"原则开始过时"到"你终于修改它"之间有3-6个月的认知滞后——你已经不再遵循这条原则了，但因为没有意识到变化，它还留在你的操作系统里造成虚假张力。
>
> **预测价值：** 把认知滞后从3-6个月压缩到2-4周。在原则被正式修改前发现征兆，主动推送"这条原则可能即将需要修订"。
>
> **已知风险：** 过度预测导致用户被频繁提醒"你要变了"——通过信号强度阈值+每月最多推送限制控制。
...
> **设计推理："成长vs漂移"区分**
>
> **解决的问题：** 不是所有变化都是好的——有"成长"（有意识地深化/精确化），也有"漂移"（被环境无意识地改变）。系统需要帮用户区分这两种变化。
>
> **为什么4个维度：** 驱动力（有没有明确原因）、方向性（精确化还是模糊化）、意识度（你知道自己在变吗）、一致性（与内核冲突吗）。4个维度中3个指向漂移→值得警惕。
>
> **设计哲学：** 成长=好的，系统记录并庆祝。漂移=需要注意的，系统指出并询问"你想继续这个方向吗？"
...
> **设计推理：维度体系的核心挑战**
>
> **解决的问题：** 维度体系面临四个互相矛盾的约束——太粗=伪模式（不相关的决策被归为一类），太细=永远积累不到5条（模式引擎无法工作），太死=现实不在列表里（无法分类新类型的决策），太活=语义漂移（同一个维度含义随时间变化）。
>
> **为什么选择三层结构：** 第一层（Domain）预设提供稳定锚点；第二层（Axis）半涌现保证覆盖度；第三层（Context Tag）完全涌现提供检索灵活性。**只用前两层做聚类和模式发现**——防止"情境标签太多导致每条决策独一无二"。
>
> **否决方案：**
> - 纯预设维度——现实比你能预想的复杂，必然遗漏
> - 纯涌现维度——冷启动阶段没有足够数据涌现
> - 关键词匹配——关键词无法抓住本质权衡（"辞职"和"是否接受offer"关键词不同但都是"稳定vs机会"的权衡）
...
> **设计推理：选择结构匹配**
>
> **解决的问题：** 关键词匹配会把"周末学英语还是学编程"和"辞职去创业"分到不同维度，但它们本质上都是"确定性vs机会成本"的权衡。需要更深层的匹配方式。
>
> **为什么"选择结构"匹配：** 两条决策被认为是"同一维度"，不是因为它们用了相似的词，而是因为它们面对的权衡结构相似——[放弃什么类型价值, 获得什么类型价值]。
>
> **否决方案：** 关键词匹配——无法抓住本质权衡，只能抓住表面描述。
...
> **设计推理：信任圈交互模式翻转**
>
> **解决的问题：** 原始方案是"系统发消息给对方→对方在系统里回复→系统整合"。但对方凭什么要在你的系统里花时间？像社交软件回复消息一样，没有回复的动机。
>
> **为什么翻转为"你事后录入"：** 对方不需要碰你的系统，甚至不需要知道系统存在。他们只是在正常跟你交流，你的系统负责把交流中的价值沉淀下来。对方负担=零。
>
> **否决方案：** 对方在系统中操作——对方无动机/像社交软件回消息/浪费他人时间。
...
> **设计推理：Lifeline的稀缺性设计**
>
> **解决的问题：** 如何在"获取外部高质量判断"的同时不消耗社交关系？如果随时可以向别人求助，你会过度依赖外部判断，同时对方也会感到负担。
>
> **核心洞察：** 当一个问题足够重要、足够稀有时，"你来问我"本身就变成了一种给予——你给了对方"被信任"的价值感。不是浪费他的时间，是在告诉他：在我人生最重要的十字路口，我认为你的判断力值得我依赖。
>
> **为什么设系统门槛保护稀缺性：** 稀缺性=社交价值。如果频繁使用，"受宠若惊"效应消失，变成"又来了"。每人每年1-2次确保每次使用都是珍贵的。
>
> **否决方案：** 随时可问——消灭稀缺性=消灭社交价值。
...
> **设计推理：可读性设计的必要性**
>
> **解决的问题：** 原则库随时间增长后（50+条原则、100+条假说、数百条决策记录），用户可能失去"我还掌控着这个系统"的感觉。复杂系统如果不做可读性管理，最终会被用户放弃。
>
> **为什么六大机制协同而非单一方案：** 可读性不是单一问题——它涉及信息呈现（三层视图）、透明度（追踪链）、信任度（确认度标记）、注意力保护（预算）、自主权（Override）、定期校准（Sync Ritual）。每个机制解决不同维度的问题。
>
> **已知风险：** 六大机制本身增加了系统复杂度——通过让大部分机制在后台运行、只在必要时浮出来缓解。
...
> **设计推理：Override Protocol的哲学**
>
> **解决的问题：** 如果系统在用户override时反驳或追问，会让用户觉得"系统在教训我"，破坏使用体验。但如果从不回应，可能错过系统需要更新逻辑的信号。
>
> **为什么"不反驳不追问"但有温和反馈机制：** 核心哲学是"你做决定"——系统永远不应该让用户觉得需要"说服系统"才能做自己的选择。但连续覆盖同类>5次是一个信号（可能系统的匹配逻辑有问题），所以轻声问一次，然后不再问。
>
> **已知风险：** 用户可能通过频繁override让系统完全无法学习——但这本身也是用户的选择权，系统尊重。
...
> **设计推理：双管线模型**
>
> **解决的问题：** 用户看信息源时只关注"这对我的决策有什么用"（内容层面）。但同一信息源可能还包含"能优化系统本身运行逻辑"的知识（元层面）。如果系统只有单管线（只提取假说），系统本身的方法论永远停留在v1.0。
>
> **核心洞察：** 同一输入源，两条完全不同的处理管线——用户管线产出假说，系统管线产出系统更新提案。系统也需要演化，不只是用户的原则在演化。
>
> **否决方案：** 单管线只提取假说——系统本身也需要演化，不应该只帮用户学习而系统自己不学习。
>
> **已知风险：** 系统自主学习可能引入不受控的变化——通过严格的审批机制（用户必须批准每条更新）和边界控制（每周最多3条提案、所有更新可逆）缓解。
...
> **设计推理：冷启动策略**
>
> **解决的问题：** 用户个人决策沉淀几乎为零，系统不能依赖"已有大量数据"来启动。需要一个低门槛、高信心的30天启动方案，让用户从"每天记2条"开始，逐步看到系统的价值。
>
> **为什么"先记录30天再涌现原则"：** 冷启动的正确方式不是从信息源提炼原则，而是设置决策实验——每天主动记录选择和理由。30天后有30+条带完整决策逻辑的记录，模式引擎才有足够数据工作。原则系统从自己的行为里长出来，不是从别人的文章里抄来的。
>
> **用户确认的冷启动洞察：** 接受了"先记录30天再搭建原则库"的策略——这意味着前10天系统不输出任何分析，用户需要有耐心。通过"每天至少2个决策记录"的极低门槛降低心理负担。
...
> **设计推理：为什么保留设计决策记录**
>
> **解决的问题：** 设计文档只展示最终方案。但理解"为什么这样设计"需要知道"考虑过什么其他方案以及为什么否决"。这个记录表就是系统设计层面的"原则考古学"。
...
*文档结束。Personal Principle OS v1.0 全层设计完成——详细阅读版（含设计推理注释）。*
